const { generateCoverLetter } = require("../src/services/openai");
...
callApi().then((res) => {
    ...
    // invalid api key => "message":"Request failed with status code 401"
    console.log('Error in catch block:', err.message)
    console.log('stringifying error: ', JSON.stringify(err))
});